...
module.exports = class Client {
  ...
  * post(api, data, opts) {
   ...
   */
  * getAccessToken() {
    const record = yield this.cache.get('accessToken');
    let accessToken = (record && record.value) || null;
    if (!accessToken) {
   ...
   */
  * getJSApiTicket() {
    const record = yield this.cache.get('jsapiTicket');
    let jsapiTicket = (record && record.value) || null;
    if (!jsapiTicket) {
