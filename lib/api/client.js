...
module.exports = class Client {
  ...
  get cache() {
    if (!this[CACHE]) {
      if (this.options.cache) {
        ...
        this[CACHE] = {
          get(key) {
            if (this[LRU][key] && (this[LRU][key].expired > Date.now())) {
              return this[LRU][key];
            }
            return {};
          },
  ...
  * post(api, data, opts) {
   ...
   */
  * getAccessToken() {
    let { value: accessToken } = yield this.cache.get('accessToken');
    if (!accessToken) {
   ...
   */
  * getJSApiTicket() {
    let { value: jsapiTicket } = yield this.cache.get('jsapiTicket');
    if (!jsapiTicket) {
