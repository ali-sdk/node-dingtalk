...
module.exports = class Client {
  ...
  async getAccessToken(options) {
    ...
    let accessToken = await this.callFn(this.cache.get, [ cacheKey ], this.cache);
    if (!accessToken) {
      let url;
      let data;
      if (appid) {
  ...
  async getJSApiTicket() {
    let jsapiTicket = await this.callFn(this.cache.get, [ 'jsapiTicket' ], this.cache);
    if (!jsapiTicket) {
