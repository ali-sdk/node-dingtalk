...
module.exports = class Client {
        ...
        this[CACHE] = {
          get(key) {
            if (LRU_CACHE[key] && (LRU_CACHE[key].expired > Date.now())) {
              return Promise.resolve(LRU_CACHE[key].value);
            }
  ...
  * callFn(fn, args) {
    ...
    if (is.generatorFunction(fn)) {
      return yield fn([].concat(args));
    }
    const r = fn([].concat(args));
    if (is.promise(r)) {
      return yield r;
    }
    return r;
  ...
  * post(api, data, opts) {
   ...
   */
  * getAccessToken() {
    let accessToken = yield this.callFn(this.cache.get, [ 'accessToken' ]);
    if (!accessToken) {
      ...
      yield this.cache.set('accessToken', accessToken, accessTokenExpireTime);
      return accessToken;
    }
    return accessToken;
  }
   ...
   */
  * getJSApiTicket() {
    let jsapiTicket = yield this.callFn(this.cache.get, [ 'jsapiTicket' ]);
    if (!jsapiTicket) {
