...
module.exports = class Extcontact {
  ...
  * list(params = {}, opts) {
    ...
    const res = yield this.client.get('topapi/extcontact/list', params, opts);
    return res.results;
  }
  ...
  * listAll(opts) {
    ...
    do {
      ...
      const users = yield this.list(params, opts);
      count = users.length;
      page += 1;
  ...
  * create(params, opts) {
    ...
    return yield this.client.post('topapi/extcontact/create', { contact: params }, opts);
  }
  ...
  * update(params, opts) {
    ...
    return yield this.client.post('topapi/extcontact/update', { contact: params }, opts);
  }
  ...
  * get(userid, opts) {
    ...
    // Tips: 目前接口使用的是 user_id 和 userid 未统一名称
    const res = yield this.client.get('topapi/extcontact/get', { user_id: userid }, opts);
    return res.result;
  }
  ...
  * delete(userid, opts) {
    assert(userid, 'user_id required');
    // Tips: 目前接口使用的是 user_id 和 userid 未统一名称
    return yield this.client.post('topapi/extcontact/delete', { user_id: userid }, opts);
  }
   ...
   */
  * listlabelgroups(params = {}, opts) {
    params = Object.assign({
      offset: 0,
      size: 20,
    }, params);

    const res = yield this.client.get('topapi/extcontact/listlabelgroups', params, opts);
    return res.results;
  }
};
