...
module.exports = class User {
  ...
  * listAll(departmentId, isSimple, opts) {
    ...
    for (const departmentId of departmentIdList) {
      ...
      let offset = 0;
      while (hasMore) {
        const response = yield this.list(departmentId, isSimple, Object.assign({ size: 100 }, opts, { offset }));
  ...
  * create(opts) {
    ...
    assert(opts.department, 'options.department, Array<Number>');
