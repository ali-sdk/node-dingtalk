...
module.exports = class Media {
  ...
  * upload(type, filePath) {
    ...
    return yield this.client.upload(url, { field: 'media', path: filePath });
  }
   ...
   */
  * get(id) {
    const response = yield this.client.get('media/get', { media_id: id });
   ...
   */
  * download(id, targetDir, fileName) {
    const url = yield this.get(id);
    const filePath = path.join(targetDir, fileName ? fileName : path.basename(url));
    if (url) {
      yield this.client.urllib.request(url, {
